...
int chek_b(double** A, double* b, int N) { //Проверка столбца свободных коэффициентов на 0
	int i;
	int count0 = 0;
	...
	if (count0 != 0) return 0;// Есть уравнение вида 0=0, решений слу бесконечно много
	return 1;
}
...
void GetX(double** M, int N, double* b,double* x) { // Обратный ход
	...
	for (int i = N - 2; i >= 0; i--)
	{
		...
		for (int j = i + 1; j < N; j++)
		{
			rem = rem + M[i][j] * x[j]; //Что отнимаем от свободного коэф.
		}
		x[i] = (b[i] - rem) / M[i][i];
	}
}
