...
int main() {
	...
	Matrix Rem(N, 1);
	p = (1 - sqrt(L1 / L2)) / (1 + sqrt(L1 / L2));//Вычисление количества итераций
	k = log(2 / e) / log(1 / p);
	k = abs(k) + 1;
	fout << k;

	int nk = abs(log(k) / log(2));//Новый размер массива для работы с опт.парам.
	k = nk;
	nk = 1;
	for (int i = 0; i <= k; i++) {
		nk = 2 * nk;
	...
	int* NK = new int[nk];//Номера параметров
